...
def createPCA_fromClstokens(predicted_list_epoch, label_list_epoch, cls_tokens_epoch, num_epochs, epoch, mode, isValLossMin=False):
    ...
    plt.scatter(pca_result[:, 0], pca_result[:, 1], c=colors, s=2)
    ...
    # 画像保存
    if (isValLossMin):
        save_path = f"/home/miyatamoe/ドキュメント/研究/1class_TF_and_NN/フォントIDの研究/Transformer/result/images/cls_tokens/{mode}/ValLossMin/cls_tokens_pca_epoch{num_epochs}_ValLossMin_no_font1kind_arranged.png"  # 保存先のパス
    else:
        save_path = f"/home/miyatamoe/ドキュメント/研究/1class_TF_and_NN/フォントIDの研究/Transformer/result/images/cls_tokens/{mode}/cls_tokens_pca_epoch{epoch+1}_no_font1kind_arranged.png"  # 保存先のパス
