...
# autoen_ep = 10000
# load_model_AE_path = f'/home/miyatamoe/ドキュメント/研究/1class_TF_and_NN/AutoEncoder/result/model/train_model/sample/Image_Encoder/Auto_Encoder/batch64_lr0.0001/per_1000epoch/model_AE_epoch{autoen_ep}.pth'

# MLPモデルの定義
class Embedding_Mlp(nn.Module):
    def __init__(self, in_size, hidden_size=None, out_size=None, act_layer=nn.GELU, drop=0.1):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(
        ...
        # self.autoencoder.load_state_dict(torch.load(load_model_AE_path))
        # self.feature_extractor = self.autoencoder.enc
    ...
    def forward(self, x, len_list):
        ...
        y = self.head1(x[:, :1, :]).squeeze()
        y = self.sig(y) # Binary追加
        # return y
        return y, x[:, :1, :].squeeze(1)
